...
class TextProcessor:
    def __init__(self):
        ...
        self.abbreviations = {
            ...
            r"\bProf\.?(?=\s|$|[,;:])": "Professor", r"\bHon'ble\.?(?=\s|$|[,;:])": "Honourable",
            r"\bSr\.?(?=\s|$|[,;:])": "Senior", r"\bJr\.?(?=\s|$|[,;:])": "Junior",
            r"\bSt\.?(?=\s|$|[,;:])": "Saint", r"\bRev\.?(?=\s|$|[,;:])": "Reverend",
            r"\bFr\.?(?=\s|$|[,;:])": "Father", r"\bSmt\.?(?=\s|$|[,;:])": "Srimati",
            r"\bSh\.?(?=\s|$|[,;:])": "Shri", r"\bEr\.?(?=\s|$|[,;:])": "Engineer",
            r"\bAr\.?(?=\s|$|[,;:])": "Architect", r"\bCol\.?(?=\s|$|[,;:])": "Colonel",
            r"\bGen\.?(?=\s|$|[,;:])": "General", r"\bCapt\.?(?=\s|$|[,;:])": "Captain",
            r"\bMaj\.?(?=\s|$|[,;:])": "Major", r"\bLt\.?(?=\s|$|[,;:])": "Lieutenant",
            r"\bSgt\.?(?=\s|$|[,;:])": "Sergeant", r"\bGovt\.(?=\s|\b)": "Government",
            r"\bDept\.(?=\s|\b)": "Department", r"\bOrg\.(?=\s|\b)": "Organization",
            r"\bUniv\.(?=\s|\b)": "University", r"\bLtd\.(?=\s|\b)": "Limited",
            r"\bPvt\.(?=\s|\b)": "Private", r"\bDist\.(?=\s|\b)": "District",
            r"\bHwy\.(?=\s|\b)": "Highway", r"\bAve\.(?=\s|\b)": "Avenue",
            r"\bRd\.(?=\s|\b)": "Road", r"\bInc\.(?=\s|\b)": "Incorporated",
            r"\bCo\.(?=\s|\b)": "Company", r"\bBros\.(?=\s|\b)": "Brothers",
            r"\bEst\.(?=\s|\b)": "Established", r"\bMfg\.(?=\s|\b)": "Manufacturing",
            r"\bRegd\.(?=\s|\b)": "Registered", r"\bJan\.(?=\s|\b)": "January",
            r"\bFeb\.(?=\s|\b)": "February", r"\bMar\.(?=\s|\b)": "March",
            r"\bApr\.(?=\s|\b)": "April", r"\bJun\.(?=\s|\b)": "June",
            r"\bJul\.(?=\s|\b)": "July", r"\bAug\.(?=\s|\b)": "August",
            r"\bSept\.(?=\s|\b)": "September", r"\bOct\.(?=\s|\b)": "October",
            r"\bNov\.(?=\s|\b)": "November", r"\bDec\.(?=\s|\b)": "December"
        }
